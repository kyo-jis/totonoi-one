...
## アクセシビリティの基準（2026-09-01 適用）

learnly のみ先行適用。効果を確認したうえで他アプリへ展開する。

- **文字サイズ**: 補助テキストの下限は `.69rem`（11px）。それ未満を追加しない
- **タップ領域**: 操作要素は実効24×24以上（WCAG 2.2 AA）。見た目を保ったまま広げるため、
  `.btn-icon-sm` `.summary-toggle` `.color-swatch` `.icon-preset` `.dur-btn` は
  `::after` で44×44の当たり判定を敷いている（`position:relative; z-index:2` が前提）
- **色**: 色を「文字」に使う場合は `--accent-text` `--green-text` `--yellow-text` `--red-text` を使う。
  `--accent` `--green` などは**背景・枠線専用**。ライトテーマでは文字用トークンだけ暗くして
  背景に対し4.5:1以上を確保している（ダークは逆に少し明るく）
- **フォーカス**: `:focus-visible` の共通アウトラインあり（マウス・タップ時は非表示）
